...
class EmailService {
    constructor() {
        this.fromEmail = 'CTG Escalerilla <escalerilla@clubdetenisgraneros.cl>';
    }
    async sendChallengeNotification(challengerName, challengedName, challengedEmail) {
        try {
    ...
    async sendAcceptedNotification(challengerName, challengedName, challengerEmail) {
        try {
            await resend.emails.send({
                from: this.fromEmail,
                to: challengerEmail,
    ...
    async sendRejectedNotification(challengerName, challengedName, challengerEmail) {
        try {
            await resend.emails.send({
                from: this.fromEmail,
                to: challengerEmail,
    ...
    async sendResultConfirmedNotification(playerName, opponentName, playerEmail, score, won, newPosition) {
        try {
            const subject = won
                ? '🏆 ¡Ganaste el partido!'
